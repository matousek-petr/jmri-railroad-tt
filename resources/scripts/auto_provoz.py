...
class dcc_basic_methods(jmri.jmrit.automat.AbstractAutomaton):
    ...
    # Nastaveni vyhybky do pozadovaneho smeru
    def setTurnout(self, turnout, state):
        for i in range(len(turnout)):
            while(LT[turnout[i]].getState() != (state[i])):
                LT[turnout[i]].setState(state[i])
                self.waitMsec(cekani_vyhybka)

    # Vyhrazeni bloku s prioritou, pokud je volny
        ...
        while a < b:
            ...
            while free:
                ...
                for i in range(len(block)):
                    if(LS[block[i]].getKnownState() == ACTIVE or 
                       IS[block[i]].getKnownState() == ACTIVE or 
                       powermanager.getPower() != jmri.PowerManager.ON or 
                       (enable.getKnownState() == ACTIVE and en == True)):
                        free = True
                        if(stop == False):
    ...
    def blockFree(self, sensor, enable = default_enable, en = False):
        for i in range(len(sensor)):
            self.waitSensorActive(LS[sensor[i]])
    ...
    def trackSelect(self, block, vj, odj):
        ...
        while free:
            ...
            free = True
            for i in range(len(block)):
                if(LS[block[i]].getKnownState() != ACTIVE and IS[block[i]].getKnownState() != ACTIVE and powermanager.getPower() == jmri.PowerManager.ON):
                    free = False 
                    a = i
                    break
                else:
...
class dcc_basic_routes(dcc_basic_methods):
 ...
 # Zabreh - smer B
    ...
    def zabreh_3_odj_b(self): self.trainOut([16, 15, 11, 10, 9],  [CLOSED, THROWN, THROWN, THROWN, CLOSED],   [37, 34, 33, 28], 4, True)
    def zabreh_4_odj_b(self): self.trainOut([14, 10, 9],  [THROWN, CLOSED, CLOSED],   [33, 28], 4, True)
    def zabreh_5_odj_b(self): self.trainOut([17, 12,  16, 15, 11, 10, 9],    [CLOSED, THROWN, THROWN, THROWN, THROWN, THROWN, CLOSED], [37, 34, 33, 28], 4, True)
    # vjezd
